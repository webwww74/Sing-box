...
#ExecStart=/etc/sing-box/argo tunnel --url http://localhost:8001 --no-autoupdate --edge-ip-version auto --protocol http2 
...
NoNewPrivileges=yes
TimeoutStartSec=0
ExecStart=/etc/sing-box/argo tunnel --url http://localhost:8001 --no-autoupdate --edge-ip-version auto --protocol http2 > /etc/sing-box/argo.log
Restart=on-failure
RestartSec=5s
...
description="Cloudflare Tunnel"
#!command="/etc/sing-box/argo tunnel --url http://localhost:8001 --no-autoupdate --edge-ip-version auto --protocol http2 > /etc/sing-box/argo.log"
#!command="/etc/sing-box/argo tunnel --url http://localhost:8001 --no-autoupdate --edge-ip-version auto --protocol http2"
...
EOF
                ...
                if command_exists rc-service 2>/dev/null; then
					...
					sed -i '/^ExecStart=/c ExecStart=/etc/sing-box/argo tunnel --edge-ip-version auto --config /etc/sing-box/tunnel.yml run 2>&1' /etc/systemd/system/argo.service
                fi
                restart_argo
                sleep 1 
                change_argo_domain
                ...
                if command_exists rc-service 2>/dev/null; then
					...
					sed -i '/^ExecStart=/c ExecStart=/etc/sing-box/argo tunnel --edge-ip-version auto --no-autoupdate --protocol http2 run --token '$argo_auth' 2>&1' /etc/systemd/system/argo.service
                fi
                restart_argo
                sleep 1 
                change_argo_domain
